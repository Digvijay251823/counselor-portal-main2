...
function Filter({
  ...
  return (
    <div className="ml-2">
      ...
      <div
        ref={componentRef}
        className={`absolute w-full lg:right-40 right-0 left-0 transition-all duration-500 z-[2500] ${
...
function ActionFilter({
  ...
  } else if (category === "counselorInitiatedName") {
    return <CounselorInitiatedName />;
    ...
    <div
      ...
      className={`rounded-xl pr-4 py-2 text-lg border transition-all duration-500 w-full flex items-center ${
        state.theme.theme === "LIGHT"
          ? `${
              onFocusFilterInput
                ? "border-gray-600 ring-4 ring-blue-100 bg-white"
                : "bg-white border-gray-300"
            }`
          : `${
              onFocusFilterInput
                ? "border-gray-600 ring-4 ring-blue-950 bg-stone-950"
                : "bg-stone-950 border-stone-800"
            }`
      }`}
    >
  ...
  return (
    <div className="relative inline-block text-left w-full" ref={menuRef}>
      <button
        ...
        onClick={() => toggleSelection(!isSelectionOpen)}
      >
        {selectedOption === "" ? "Select Marital Status" : selectedOption}
        <ChevronDownIcon className="h-4 w-4" />
      </button>
      {isSelectionOpen && (
        <div
...
function CounselorInitiatedName() {
  const { state } = useGlobalState();
  const [onFocusFilterInput, setOnFocusFilterInput] = useState(false);
  const [searchParamsInput, setSearchParamsInput] = useState("");
  const initialRef = useRef(true);
  ...
  return (
    ...
    >
      ...
      <input
        ...
        value={searchParamsInput}
        type={"text"}
        placeholder={`Search Counselor Initiated Name`}
        className={`outline-none w-full ${
          state.theme.theme === "LIGHT"
            ? `transition-all duration-500  ${
                onFocusFilterInput
